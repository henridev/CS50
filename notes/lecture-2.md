...
- Strings = arrays of characters.  each character can be accessed with `s[0]`, `s[1]` ...

- string ends with a special character, **‘\0’, or a byte with all bits set to 0**. This character is called the null character, or null terminating character. --> so we need 4 instead of 3 bytes to store the string 
